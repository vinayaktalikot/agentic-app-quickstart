...
class DataManager:
    ...
    def _validate_numeric_column(self, df: pd.DataFrame, column_name: str) -> None:
        ...
        if not pd.api.types.is_numeric_dtype(df[column_name]):
            raise ValueError(
                f"Column '{column_name}' is not numeric. Data type: {df[column_name].dtype}"
            )
...
def count_rows_with_value(
    filename: str, column_name: str, value: str | int | float
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        percentage = (
            round((matching_rows / total_rows) * 100, 2) if total_rows > 0 else 0
        )
...
def perform_data_aggregation(
    filename: str, group_by_column: str, operation: str, target_column: str
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        if operation not in ["sum", "mean", "count", "min", "max", "median"]:
            return {
                "error": f"Unsupported operation '{operation}'. Supported: sum, mean, count, min, max, median"
            }
        ...
        aggregated_data = [
            {
                "group_value": str(key),
                "result": (
                    round(float(value), 2) if isinstance(value, (int, float)) else value
                ),
            }
...
@function_tool
def find_outliers(
    filename: str, column_name: str, method: str = "iqr"
) -> dict[str, Any]:
    """
    ...
    try:
        ...
        return {
            ...
            "outlier_percentage": round(
                (len(outlier_values) / len(column_data)) * 100, 2
            ),
...
def _generate_correlation_insights(correlations: list[dict]) -> list[str]:
    ...
    for corr in correlations:
        if abs(corr["correlation"]) >= 0.7:
            ...
            insights.append(
                f"Strong {direction} correlation ({corr['correlation']}) between {corr['column1']} and {corr['column2']}"
            )
        elif abs(corr["correlation"]) >= 0.4:
            direction = "positive" if corr["correlation"] > 0 else "negative"
            insights.append(
                f"Moderate {direction} correlation ({corr['correlation']}) between {corr['column1']} and {corr['column2']}"
            )
